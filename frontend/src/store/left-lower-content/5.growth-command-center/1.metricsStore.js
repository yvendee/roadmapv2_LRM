// frontend\src\store\left-lower-content\5.growth-command-center\1.metricsStore.js
...
const useMetricsStore = create((set) => ({
  ...
  updateMetric: (index, updatedMetric) =>
    set((state) => {
      const currentMetric = state.metrics[index];
  
      const hasChanged = Object.entries(updatedMetric).some(
        ([key, value]) => currentMetric[key] !== value
      );
      ...
      newMetrics[index] = { ...currentMetric, ...updatedMetric };
